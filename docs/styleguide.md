...
- Если Вам нужно привести тип с помощью `as` (или перебрать в цикле `for`), делайте это только до такого типа, в котором есть нужный(-ая) Вам метод/переменная/сигнал.
