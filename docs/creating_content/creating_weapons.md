...
**Заметка**: это инструкция по созданию *оружия*. Если Вы хотите создать *тип оружия*, обратитесь к соответствующей секции документа.
...
2. В этой папке создайте ресурс типа `WeaponData`, назовите именем Вашего оружия (всё ещё в *snake_case*) и заполните в нём информацию:
    ...
    - В `Rarity` укажите редкость оружия. Как определяется редкость см. `giving_rarity.md`.
    ...
    - Во вкладке `Stats`:
        - В `Damage Text` кратко укажите, какой урон наносит оружие.
            - Для большинства рекомендуется формат "Урон/с: ХХ", однако для оружия, делающего редкие выстрелы (секунда и более), или одноразового оружия (гранаты) можно просто указать "Урон: ХХ".
...
3. Нарисуйте оружие. Не забывайте про принципы в `drawing.md`, а также про ещё несколько моментов:
    ...
    - Настоятельно рекомендуется использовать [Krita](https://krita.org) в качестве программы для рисования.
