...
func _show_data() -> void:
	...
	var data: Array[int] = Globals.get_variant("preset_%d_data" % idx, [] as Array[int])
	(%Skill as TextureRect).texture = load(Globals.items_db.skills[data[0]].image_path)
	(%Light as TextureRect).texture = load(Globals.items_db.weapons_light[data[1]].image_path)
	(%Heavy as TextureRect).texture = load(Globals.items_db.weapons_heavy[data[2]].image_path)
	(%Support as TextureRect).texture = load(Globals.items_db.weapons_support[data[3]].image_path)
	(%Melee as TextureRect).texture = load(Globals.items_db.weapons_melee[data[4]].image_path)
...
func _on_delete_pressed() -> void:
	...
	Globals.set_variant("preset_%d_data" % idx, [] as Array[int])
	_show_empty()
